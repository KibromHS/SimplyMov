...
public class MoveNowActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_move_now);
        initView();
        ...
        bookBtn.setOnClickListener(view -> {
            if (checkedTruckSize == null) {
                Toast.makeText(MoveNowActivity.this, "Please select truck size", Toast.LENGTH_SHORT).show();
                return;
            }
            ...
            LatLng pickup = MapUtil.geocodeAddress(MoveNowActivity.this, from);
            if (pickup == null) {
                Toast.makeText(MoveNowActivity.this, "Modify the pickup location", Toast.LENGTH_LONG).show();
                return;
            }
            LatLng dropOff = MapUtil.geocodeAddress(MoveNowActivity.this, to);
            if (dropOff == null) {
                Toast.makeText(MoveNowActivity.this, "Modify the drop off location", Toast.LENGTH_LONG).show();
                return;
            }
    ...
    private void bookOrder() {
        progressDialog = new ProgressDialog(MoveNowActivity.this);
        ...
        db.collection("orders").add(data).addOnSuccessListener(documentReference -> {
            ...
            Toast.makeText(MoveNowActivity.this, "Order added successfully!", Toast.LENGTH_SHORT).show();

            Intent i = new Intent(MoveNowActivity.this, MainActivity.class);
            startActivity(i);
            finish();
        });
    }
    ...
    private void showCustomDialog() {
        ...
        orderDate.setText(FirebaseUtil.timestampToDateString(Timestamp.now()));
        orderTime.setText(FirebaseUtil.timestampToTimeString(Timestamp.now()));
